...
if ($slug !== null) {
	...
	$slug = '';
		  ...
		  if($text !== null){
			...
			$input = '<input type="text" id="slug" name="slug" autocomplete="off" value="' . $slug . '" class="mono" />';
			?>
			...
			<p>
				<label for="text" class="sr-only"><?php _e('文章内容'); ?></label>
                             <textarea style="height: <?php $options->editorSize(); ?>px; margin-top:5px;" autocomplete="off" id="text" name="text" class="w-100 mono"><?php echo $text; ?></textarea>
			</p>
			...
			<div id="advance-panel">
				...
				<section class="typecho-post-option visibility-option">
					...
					<select id="visibility" name="visibility">
						...
						<?php $pass = $post->password; if($pass !== null){ $pass = strlen($post->password);}else{$pass = '';} ?>	
						<option value="password"<?php if ($pass > 0): ?> selected<?php endif; ?>><?php _e('密码保护'); ?></option>
					...
					<p id="post-password"<?php if ($pass == 0): ?> class="hidden"<?php endif; ?>>
